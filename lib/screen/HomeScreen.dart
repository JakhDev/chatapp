...
class _HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    _searchSlide = Tween<Offset>(
        ...
        parent: _searchAnimCtrl, curve: Curves.easeOutCubic));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final unreadCount = context.watch<ChatProvider>().chats.fold<int>(
      0,
...
class _ChatsScreen extends StatelessWidget {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child:
            Text('Bekor qilish', style: TextStyle(color: c.textSecondary)),
          ),
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
  ...
  String _fmt(DateTime? dt) {
    ...
    if (tDate == today.subtract(const Duration(days: 1))) {
      return 'Kecha';
    }
  ...
  Widget build(BuildContext context) {
        ...
        child: Column(
          ...
          children: [
            TweenAnimationBuilder<double>(
              ...
              child:
              Icon(Icons.chat_bubble_outline, size: 64, color: c.textSecondary),
            ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        // FIX 2: avatar — allUsers dan olish (Google photo)
        final otherUser = isGroup
            ? null
            : provider.allUsers
            .where((u) => u.id.toLowerCase() == otherId.toLowerCase())
            .firstOrNull;
        final avatarUrl = otherUser?.avatarUrl ?? chat.avatarUrl;
                        ...
                        Positioned(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color:
                                  AppColorsContext.of(context).background,
                                  width: 2),
                            ),
                          ),
                        ),
...
// FIX 2: Universal avatar widget — Google photo support
    ...
    if (avatarUrl != null && avatarUrl!.isNotEmpty) {
      return Container(
        ...
        child: Image.network(
          ...
          width: size,
          height: size,
          // FIX 2: Cache headers — Google photo CORS fix
...
class _ContactsScreenState extends State<_ContactsScreen> {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            // FIX 3: ID ko'rsatish contacts profileda ham
  ...
  Widget build(BuildContext context) {
    ...
    final users = provider.allUsers
        .where((u) => u.id.toLowerCase() != myId.toLowerCase())
        .toList();
          ...
          children: [
            TweenAnimationBuilder<double>(
              ...
              builder: (_, v, child) =>
                  Transform.scale(scale: v, child: child),
              child:
              Icon(Icons.people_outline, size: 64, color: c.textSecondary),
            ),
            const SizedBox(height: 14),
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        final u = users[i];
        final isOnline =
        context.watch<ChatProvider>().isUserOnline(u.id);
        ...
        return TweenAnimationBuilder<double>(
          ...
          child: ListTile(
            ...
            leading: Stack(
              children: [
                // FIX 2: Google avatar contacts da
                _UserAvatar(
                  name: u.name,
            ...
            subtitle: isOnline
                ? const Text('Onlayn',
                style:
                TextStyle(color: AppColors.online, fontSize: 11))
                : null,
            onTap: () async {
              final chat =
              ...
              Navigator.push(
                ...
                if (ctx.mounted) {
                  ctx.read<ChatProvider>().closeChat();
                }
              });
            },
            onLongPress: () => _showUserProfile(u, AppColorsContext.of(context)),
          ),
        );
      },
    );
      ...
      children: [
        ...
        Padding(
          ...
          child: Container(
            ...
            child: Column(children: [
              ...
              _infoTile(Icons.fingerprint_rounded, 'ID',
        ...
        Padding(
          ...
          child: OutlinedButton(
            ...
            onPressed: () => _confirmLogout(context, s, c),
            child: Text(_getLogoutButton(s),
        ...
        Padding(
          ...
          child: TextButton(
            ...
            onPressed: () => _confirmDeleteAccount(context, s, c),
            child: Text(_getDeleteAccountLabel(s),
                style: TextStyle(
                    color: c.textSecondary,
                    fontSize: 14,
...
// ── Logout dialog ─────────────────────────────────────────────────────────
  void _confirmLogout(
      BuildContext context, AppSettings s, ThemeColors c) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: c.surface,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text(_getLogoutTitle(s),
            style: TextStyle(color: c.textPrimary)),
        content: Text(_getLogoutMessage(s),
            style: TextStyle(color: c.textSecondary)),
        actions: [
          TextButton(
    ...
    // ✅ Login ekraniga tezda o'tamiz
    if (context.mounted) {
      Navigator.of(context).pushNamedAndRemoveUntil(
        '/login',  // yoki '/splash' routing nomiga qarab
            (route) => false,
      );
    }
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child:
            Text(_getNo(s), style: TextStyle(color: c.textSecondary)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(ctx, true),
            child: Text(_getDeleteAccountConfirm(s),
                style: const TextStyle(
                    color: Colors.redAccent, fontWeight: FontWeight.w700)),
          ),
        ],
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Row(children: [
          ...
          Text(_getDeletingLabel(s),
              style: TextStyle(color: c.textPrimary)),
        ]),
      ),
    );

    // ✅ deleteAccount() ni await bilan kutamiz
    // currentUser=null bo'lgach _AppEntry SplashScreen ga o'tadi
    await context.read<ChatProvider>().deleteAccount();

    // Loading dialogni yopish (agar hali mounted bo'lsa)
    if (context.mounted) {
      Navigator.of(context).pop();
    }
    // ✅ Qo'shimcha Navigator.pushNamed KERAK EMAS
  }

  // ── Labels ────────────────────────────────────────────────────────────────
  String _getDeleteAccountLabel(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Hisobni o\'chirish';
      case AppLanguage.ru: return 'Удалить аккаунт';
      case AppLanguage.en: return 'Delete account';
    }
  }

  String _getDeleteAccountWarning(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz:
        return 'Hisobingiz va barcha xabarlaringiz butunlay o\'chiriladi. Bu amalni qaytarib bo\'lmaydi!';
      case AppLanguage.ru:
        return 'Ваш аккаунт и все сообщения будут удалены навсегда. Это действие нельзя отменить!';
      case AppLanguage.en:
        return 'Your account and all messages will be permanently deleted. This action cannot be undone!';
    }
  }

  String _getDeleteAccountConfirm(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Ha, o\'chir';
      case AppLanguage.ru: return 'Да, удалить';
      case AppLanguage.en: return 'Yes, delete';
    }
  }

  String _getDeletingLabel(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'O\'chirilmoqda...';
      case AppLanguage.ru: return 'Удаление...';
      case AppLanguage.en: return 'Deleting...';
    }
  }

  String _getLogoutTitle(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Chiqish';
      case AppLanguage.ru: return 'Выход';
      case AppLanguage.en: return 'Logout';
    }
  }

  String _getLogoutMessage(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Dasturdan chiqmoqchimisiz?';
      case AppLanguage.ru: return 'Вы хотите выйти?';
      case AppLanguage.en: return 'Do you want to logout?';
    }
  }

  String _getLogoutButton(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Chiqish';
      case AppLanguage.ru: return 'Выход';
      case AppLanguage.en: return 'Logout';
  ...
  String _getNo(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return "Yo'q";
      case AppLanguage.ru: return 'Нет';
      case AppLanguage.en: return 'No';
    }
  }

  String _getYes(AppSettings s) {
    switch (s.language) {
      case AppLanguage.uz: return 'Ha';
      case AppLanguage.ru: return 'Да';
      case AppLanguage.en: return 'Yes';
    }
  }
  ...
  Widget _infoTile(IconData icon, String label, String value,
      Color color, ThemeColors c) =>
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        decoration: BoxDecoration(
            color: c.surfaceLight, borderRadius: BorderRadius.circular(14)),
        child: Row(children: [
          Container(
            width: 36, height: 36,
            decoration: BoxDecoration(
